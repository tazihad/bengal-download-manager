        ...
        try:
            ...
            while finished_count < num_threads and self.is_running:
                ...
                if self.is_paused:
                    ...
                    self.main_progress_signal.emit(self.row_index, (
                        self.filename, 
                        self.format_bytes(total_size, precision=2, pad=False, space_count=2) if total_size > 0 else "Unknown", 
                        "Paused", 
                        "", 
                        "0 KB/s",
                        total_dl,
                ...
                self.main_progress_signal.emit(self.row_index, (
                    ...
                    f"{self.format_bytes(total_speed, precision=4, pad=False, space_count=2)}/s",
                    total_dl,
                    total_size
                ))
            ...
            if self.is_running:
                ...
                try:
                    ...
                    self.main_progress_signal.emit(self.row_index, (self.filename, self.format_bytes(total_size, precision=2, pad=False, space_count=2) if total_size > 0 else "Unknown", "Complete", "", "", total_size, total_size))
