...
class DownloadProgressDialog(QDialog):
    ...
    def update_stats(self, row, data):
        ...
        self.lbl_downloaded.setText(f"{self.worker.format_bytes(current_bytes, precision=2, pad=False, space_count=2)} ({percent})")
    ...
    def on_finished(self, row, status):
        ...
        if display_status == "Complete":
            if self.total_bytes > 0:
                ...
                self.lbl_downloaded.setText(f"{self.worker.format_bytes(self.total_bytes, precision=2, pad=False, space_count=2)} (100.0%)")
            else:
                self.pbar.setMaximum(10000)
                self.pbar.setValue(10000)
                self.lbl_downloaded.setText(f"{self.worker.format_bytes(self.current_bytes, precision=2, pad=False, space_count=2)} (100.0%)")
