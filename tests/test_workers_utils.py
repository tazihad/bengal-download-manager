...
def test_format_bytes_precision_fetcher():
    ...
    # Default: 3 decimal places, padded with 5+3=8 spaces, 1 space before unit
    assert worker.format_bytes(1024) == "   1.000 KB"
    # Custom: 2 decimal places, no padding, 2 spaces before unit
    assert worker.format_bytes(1024, precision=2, pad=False, space_count=2) == "1.00  KB"
    # Custom: 4 decimal places, no padding, 2 spaces before unit
    assert worker.format_bytes(1024, precision=4, pad=False, space_count=2) == "1.0000  KB"
