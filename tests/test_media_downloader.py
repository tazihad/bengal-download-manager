...
def test_ytdlp_worker_migrates_incomplete_fragments_from_save_dir_to_cache(tmp_path):
    ...
    frag_file = save_dir / "rickroll.mp4.part-Frag1"
    frag_file.write_bytes(b"fragment_1_data")
    part_file = save_dir / "rickroll.mp4.part"
    part_file.write_bytes(b"partial_video_data")
    ytdl_file = save_dir / "rickroll.mp4.ytdl"
    ...
    with patch("core.media_downloader.YtDlpManager.ensure_binary", return_value=str(fake_bin)), \
        ...
        assert not (save_dir / "rickroll.mp4.part-Frag1").exists()
        assert not (save_dir / "rickroll.mp4.part").exists()
        assert not (save_dir / "rickroll.mp4.ytdl").exists()
